...
syn region cshtmlInnerHtmlBlock start="\(<\(bool\|byte\|sbyte\|char\|decimal\|double\|float\|int\|uint\|long\|ulong\|object\|short\|ushort\|string\)\@!\z([a-z]\w*\)\)" end="\(<\/\z1[^>]*>\)\|\(/>\(<\z1[^>]*\/>\)\@<=\)" extend keepend containedin=@csHtmlContainedInRazor contains=@csHtmlContainingRazor
...
syn match cshtmlCsharpBlockShort "@functions\_s*" nextgroup=cshtmlInnerRazor containedin=@csHtmlContainedInRazor
